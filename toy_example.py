   ...
   'layers': [{'id': '35wg7r',
     ...
     'config': {'dataId': 'hex_data',
      ...
      'visConfig': {'opacity': 0.72,
       'colorRange': {'name': 'Global Warming 5',
        'type': 'sequential',
        'category': 'Uber',
        'colors': ['#FFC300', '#D55D0E', '#AC1C17', '#831A3D', '#5A1846'],
        'reversed': True},
       'coverage': 0.95,
       'enable3d': True,
       'sizeRange': [0, 500],
    ...
    {'id': 'a3so98a',
     ...
     'visualChannels': {'colorField': None,
      ...
      'sizeField': {'name': 'point_score', 'type': 'integer'},
      'sizeScale': 'sqrt'}}],
   'interactionConfig': {'tooltip': {'fieldsToShow': {'hex_data': [{'name': 'h3_id',
        'format': None},
       {'name': 'h3_centroid', 'format': None},
       {'name': 'h3_score', 'format': None}],
      'point_data': [{'name': 'point_id', 'format': None},
       {'name': 'lat', 'format': None},
       {'name': 'lon', 'format': None},
       {'name': 'point_score', 'format': None}]},
     'compareMode': False,
...
def generate_city_gdf(country, city, layer, config):
    ...
    if city == 'Dubai':
        ...
        city_gdf = city_gdf[city_gdf['NAME_2'].isin(['Sector 1','Sector 2','Sector 3','Sector 4','Sector 5','Sector 6'])]
...
def generate_h3_df(city_gdf, resolution = 7):
    ...
    h3_df[['country','city','sector','h3_id', 'h3_geo_boundary','h3_centroid']] = pd.DataFrame.from_dict(data=h3_dict, orient='index')
...
def generate_point_df(h3_df, empty_hex_rate = 0.5):
    ...
    if len(point_dict) > 0:
        point_df[['point_id', 'point_score', 'lat', 'lon', 'h3_id']] = pd.DataFrame.from_dict(data = point_dict, orient='index')
